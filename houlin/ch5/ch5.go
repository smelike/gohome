...
/*
	...
	*sync.Cond 类型的方法集合有 3 个方法，即：Wait、Signal 和 Broadcast。
	...
	sync.Once 类型的典型应用场景就是执行仅需执行一次的任务。这样的任务并不是都适合在 init 函数中
	执行，这时 sync.Once 类型就派上用场了。例如，数据库连接池的建立、全局变量的延迟初始化，等等

	#5.5 WaitGroup

	sync.WaitGroup 类型的值是并发安全的。

	例如，在声明 var wg sync.WaitGroup 之后，就可以直接使用 wg 变量了。
	该类型有 3 个指针方法，即： Add、Done 和 Wait。

	sync.WaitGroup 是一个结构体类型。其中有一个代表计数的【字节数组类型】的字段，该字段用 4 字节表示给定
	计数，另用 4 字节表示等待计数。当一个 sync.WaitGroup 类型的变量被声明之后，其中的这两个计数都会是 0。
	可以通过该值的 Add 方法增大或减小其中的给定计数，例如：wg.Add(3) 或 wg.Add(-3)

	#5.6 临时对象池

	1\ sync.Pool 类型，可以把 sync.Pool 类型值看作临时存放临时值的容器。
	此类容器是自动伸缩的、高效地，同时也是并发安全的。为了描述方便，把 sync.Pool
	类型的值称为”临时对象池“，而把存于其中的值称为”对象值“。

	2\ 在用复合字面量初始化一个临时对象池的时候，可以为它唯一的公开字段 New 赋值。
	该字段的类型是 func() interface{}，即一个函数类型。
	赋给该字段的函数会被临时对象池用来创建对象值。不过，该函数一般仅在池中无可用
	对象值的时候才被调用。一般称之为”对象值生成函数“。

	如：
	var count int32
	newFunc := funbc () interface{} {
		atomic.AddInt32(&count, 1)
	}
	pool := sync.Pool{New: newFunc}

	3\ sync.Pool 类型有两个公开的指针方法：Get 和 Put。

	Get 的功能是从池中取一个 interface{} 类型的值，而 Put 的作用
	则是把一个 interface{}类型的值放置于池中。

	通过 Get 方法获取到的值是任意的。如果一个临时对象池的 Put 方法未被调用过，
	且它的 New 字段亦未曾被赋予一个非 nil 的函数值（即 New 未 nil），那它的 Get 方法
	返回的结果就一定是 nil。
	Get 方法返回的不一定就是存在于池中的值。如果这个结果值是池中的，那再该方法返回它之前，
	就一定会把它从池中删除。

	4\ 非常突出的特性
	第一，临时对象池可以把由其中的对象值产生的存储压力进行分摊。
	进一步说，它会专门为每一个与操作它的 goroutine 【相关联的 P】 建立本地池。
	在临时对象池的 Get 方法被调用时，它一般会先尝试从与【本地 P】 对应的那个本地
	私有池和本地共享池中获取一个对象值。如果获取失败，它就会试图从【其他 P】 的本地
	共享池中偷一个对象值。
	如果获取失败，它就会试图从其他 P 的本地共享池中偷一个对象值并直接返回给调用方。
	如果依然未果，它就只能把希望寄托于当前临时对象池的对象值生成函数了。
	（P 是指什么？是 pool ？ 还是 sync.Pool）

	注意：这个对象值生成函数产生的对象值永远不会被放置到池中，而是会被直接返回给调用方。

	临时对象池的 Put 方法会把它的参数值存放到[本地 P] 的本地池中。
	每个【相关 P】 的本地共享池中的所有对象值，都是在【当前临时对象池】的范围内共享。
	也就是说，它们随时可能会被偷走。（即被其他 P 拿走的意思？）

	第二，对垃圾回收友好。
	垃圾回收的执行一般会使临时对象池中的对象值全部被移除。
	也就是说，即使永不会显式地从临时对象池取走某个对象值，该对象值也不会永远待在临时
	对象池中，它的生命周期取决于垃圾回收任务下一次的执行时间。
	...
	注意：获取值的先后顺序与放入时的顺序毫无关系。（我不是那么的理解？）

	强调：不应对从【池中获取的值】有任何假设，因为它可能是池中的任何一个值，也可能是对象值生成函数产生的值。

	临时对象池的实例不应被赋值，否则 go vet 命令将报告此问题。
*/
