...
/*
	...
	// 载入发生器、调用器以及 goroutine 票池，都使用了接口类型定义其行为，而后才是编写实现。
	暴露于包外的附带方法的类型都是接口类型，而不是作为实现的具体类型。当你需要把某个函数暴露在包外时，就要考虑它的参数列表在可预见的未来是否有变化的可能。
	如果答案是肯定的，就把参数列表封装成一个结构体类型，就像 ParamSet 类型那样。


	// 基于 TCP 协议的通信时使用字节流来传递上层给予的消息的。
	它会根据具体情况为消息分段，但却无法感知消息的分界。
	因此，需要显式地为请求数据添加结束符，而传给 write 方法和 read 方法的参数 DELIM 就代表了这个结束符，这两个方法会用它来且分出单个的请求或响应。
*/
